...
class db
{
	...
	//Food Item 
	function InsertItem($conn, $table, $foodname ,$category , $description ,$price ,$image)
	{
		$stmt = $conn->prepare("INSERT INTO $table (foodname ,category , description ,price ,image) VALUES (?, ?, ?, ?, ?)");
		$stmt -> bind_param("sssss",$foodname ,$category , $description ,$price ,$image);
		$stmt -> execute();  
		$stmt -> close();
		return $stmt; 
	}

	function UpdateFoodProfile($conn,$table,$update_foodname, $update_category, $update_description, $update_price, $food_id)
	{
		$result=$conn->query("UPDATE $table SET foodname='$update_foodname',category='$update_category', description='$update_description', price='$update_price' where id='$food_id'");
		return $result;
	}

	function viewFoodProfile($conn,$table,$food_id)
	{
		$stmt = $conn -> query("SELECT id, foodname, category, description, price, image FROM ". $table." WHERE id='". $food_id."'");
		return $stmt;
	}
	...
	function DeleteItem($conn,$table,$food_id)
	{
		$result=$conn->query("DELETE FROM $table WHERE id ='$food_id'");
		return $result;
	}
